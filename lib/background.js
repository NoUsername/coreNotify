...
if (typeof require != 'undefined') {
	...
	var cnUtil = window.cnUtil;
	var brUtils = window.brUtils;
}
...
self.updateUi = function(notificationCount, htmlSummary, summary) {
	if (notificationCount > 0) {
		self.setBadge(notificationCount.toString());
	} else {
		self.setBadge("");
	}
	summary.lastCheck = currentTimeFormatted();
	// also update popups
	brUtils.withView(brUtils.getViewTabUrl(), function(view) {
		if (summary) {
			view.setContent(summary);
		} else {
			var fallback = $('<div>', {text: 'news: ' + (notificationCount)});
			view.setContent(fallback);
		}
	});
}
...
self.updateNotifications = function() {
	...
	fetchHtml(function(data) {
		if (!isLoggedIn(data)) {
			...
			self.updateUi(0, '', {'loggedIn':false});
			self.setBadge("!");
			return;
		}
...
self.handleNotifications = function(notificationCount, summary) {
	...
	self.updateUi(notificationCount, null, summary);
};
